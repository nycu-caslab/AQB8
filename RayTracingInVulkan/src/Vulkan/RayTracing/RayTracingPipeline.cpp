...
namespace Vulkan::RayTracing
{
	...
	RayTracingPipeline::RayTracingPipeline(
		...
		const BottomLevelAccelerationStructure &bottomAccelerationStructure,
		...
		const uint32_t shaderType) : swapChain_(swapChain)
	{
		...
		for (uint32_t i = 0; i != swapChain.Images().size(); ++i)
		{
			...
			// bvh_trigs buffer
			VkDescriptorBufferInfo compress_bvh_trigs_Buffer = {};
			compress_bvh_trigs_Buffer.buffer = bottomAccelerationStructure.compress_bvh_TrigsBuffer().Handle();
			compress_bvh_trigs_Buffer.range = VK_WHOLE_SIZE;
			descriptorWrites.push_back(descriptorSets.Bind(i, 12, compress_bvh_trigs_Buffer));

			// bvh_nodes buffer
			VkDescriptorBufferInfo compress_bvh_nodes_Buffer = {};
			compress_bvh_nodes_Buffer.buffer = bottomAccelerationStructure.compress_bvh_NodeBuffer().Handle();
			compress_bvh_nodes_Buffer.range = VK_WHOLE_SIZE;
			descriptorWrites.push_back(descriptorSets.Bind(i, 13, compress_bvh_nodes_Buffer));

			// bvh_root buffer
			VkDescriptorBufferInfo compress_bvh_root_Buffer = {};
			compress_bvh_root_Buffer.buffer = bottomAccelerationStructure.compress_bvh_RootBuffer().Handle();
			compress_bvh_root_Buffer.range = VK_WHOLE_SIZE;
			descriptorWrites.push_back(descriptorSets.Bind(i, 14, compress_bvh_root_Buffer));

			// bvh_primitive_indices buffer
			VkDescriptorBufferInfo compress_bvh_primitive_indices_Buffer = {};
			compress_bvh_primitive_indices_Buffer.buffer = bottomAccelerationStructure.compress_bvh_PrimitiveIndicesBuffer().Handle();
			compress_bvh_primitive_indices_Buffer.range = VK_WHOLE_SIZE;
			descriptorWrites.push_back(descriptorSets.Bind(i, 15, compress_bvh_primitive_indices_Buffer));

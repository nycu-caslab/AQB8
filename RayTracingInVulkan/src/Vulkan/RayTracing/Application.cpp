...
namespace Vulkan::RayTracing
{
	...
	void Application::CreateSwapChain()
	{
		...
		rayTracingPipeline_.reset(new RayTracingPipeline(*deviceProcedures_, SwapChain(), topAs_[0], bottomAs_[0], *accumulationImageView_, *outputImageView_, UniformBuffers(), GetScene(), GetShaderType()));
	...
	void Application::CreateBottomLevelStructures(VkCommandBuffer commandBuffer)
	{
		...
		for (size_t i = 0; i != bottomAs_.size(); ++i)
		{
			...
			bottomAs_[i].Generate(commandBuffer, CommandPool(), *bottomScratchBuffer_, scratchOffset, *bottomBuffer_, resultOffset, i);

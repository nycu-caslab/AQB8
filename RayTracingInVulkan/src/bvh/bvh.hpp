    ...
    struct Bvh
    {
        ...
        inline void convert_nodes(const std::unique_ptr<Node[]> &old_nodes, size_t node_count)
        {
            ...
            for (size_t i = 0; i < node_count; i++)
            {
                ...
                std::memcpy(new_n.left_bounds_quant, left_child.bounds_quant, sizeof(uint8_t) * 6);
                std::memcpy(new_n.right_bounds_quant, right_child.bounds_quant, sizeof(uint8_t) * 6);

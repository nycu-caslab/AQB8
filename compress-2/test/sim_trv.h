...
void stack_op(rid_t& rid,
    ...
    if (local_mem.node_stack_size == 0) {
        ...
        local_mem.all_ray.b_x = node_stack_top.b.x_min;
        local_mem.all_ray.b_y = node_stack_top.b.y_min;
        local_mem.all_ray.b_z = node_stack_top.b.z_min;
        send_mem_req(rid, local_mem_, node_stack_top.node, bbox_mem_req_stream, ist_mem_req_stream);
    }
}
...
void sim_trv(/* in  */ ac_channel<trv_req_t>& trv_req_stream,
    ...
    if (trv_req_stream.nb_read(trv_req)) {
        ...
        bbox_req.exp                   = bbox_mem_resp.nbp.exp;
        ...
        if (bbox_resp.left_hit) {
            if (bbox_resp.right_hit) {
                ...
                ref_bbox_t& first_b = left_first ? bbox_resp.left_b: bbox_resp.right_b;
                ref_bbox_t& second_b = left_first ? bbox_resp.right_b : bbox_resp.left_b;
                
                // push to node_stack
                sim_assert(local_mem.node_stack_size != STACK_SIZE);
                local_mem.node_stack_[local_mem.node_stack_size++] = {
                    .node = second_node,
                    .b = second_b
                };
                node_stack_max_size = std::max(node_stack_max_size, local_mem.node_stack_size.to_int());

                local_mem.all_ray.b_x = first_b.x_min;
                local_mem.all_ray.b_y = first_b.y_min;
                local_mem.all_ray.b_z = first_b.z_min;
                send_mem_req(bbox_resp.rid, local_mem_, first_node, bbox_mem_req_stream, ist_mem_req_stream);
            } else {
                local_mem.all_ray.b_x = bbox_resp.left_b.x_min;
                local_mem.all_ray.b_y = bbox_resp.left_b.y_min;
                local_mem.all_ray.b_z = bbox_resp.left_b.z_min;
            ...
            local_mem.all_ray.b_x = bbox_resp.right_b.x_min;
            local_mem.all_ray.b_y = bbox_resp.right_b.y_min;
            local_mem.all_ray.b_z = bbox_resp.right_b.z_min;
